...
class HomeListView(ListView):
    ...
    def get_context_data(self, *, object_list=None, **kwargs):
        ...
        # if 'device_id' not in self.request.session:
        #
        #     self.request.session['device_id']=session_id()
        #
...
def AddtoCart(request, slug, ):
    ...
    if order_exist.exists():
        ...
        if create_orderitem_table.quantity == 1:
            ...
            if request.is_ajax:
                data['inc']= True, sum(cart_total), create_orderitem_table.item.product_name, sum(grand_total), price_total, sum(item_total)
                return JsonResponse(data)
...
def payment(request):
    ...
    if verify_payment['chargecode'] == '00' and verify_payment['amount'] == verify_payment['chargedamount'] and verify_payment['error']==False:
        ...
        for orders in orderitems:
            # craete an order with the item in their cart
            order = Order.objects.create(myuser=orders.user, orderitem=orders.item, user_quantity=orders.quantity,
                                         order_id=('GS' + str(order_id)), reference_id=payment_ref, total=orders.get_price())

            order.ordered = True
            order.save()

            # order created succesffully
...
def ProcessReviews(request, slug):
    ...
    if request.method== 'POST' and request.user.is_authenticated:
        ...
        if ProductReviews.objects.filter(user=request.user, product=product).exists():
            ...
            messages.success(request, 'Hello {} you cant add more than one review on one product'.format(request.user.username))
            print('This user has rated this product')
            return redirect('product', slug=slug)
            ...
            if Rating.objects.filter(user=request.user, product=product).exists():
                ...
                for r in rate:
                    ...
                    save_user_review= ProductReviews.objects.get_or_create(user=request.user, product=product, your_review=user_review, rating=r.rated)
