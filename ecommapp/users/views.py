...
from myproject.decorators import redirect_auth_users, persist_session_vars
from django.http import JsonResponse
from userwallet.models import UserWallet
from django.contrib.auth import update_session_auth_hash
from ecommerce.models import Products, Orderitem
...
from ecommerce.views import session_id
...
def log_in(request):
    ...
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            if Orderitem.objects.filter(device_name=device).exists():
                ...
                for i in orderitem:
                    if i.user is None:
                        i.user = request.user
                        i.save()
                        orderitem = Orderitem.objects.filter(device_name=device, user=request.user)
                        print(orderitem)
